...
2026-09-06 16:22 UTC 地缘历史自动关联已上线：`GeoObservation` 由 Hub 传入 active，按共享 data/errors/busy 和稳定资产键派发最多两项；自动流程无持久队列，视图隐藏或切事件淘汰未开始需求。全部手动历史入口在已有请求期间禁用。自动调度、错误/空数据重试与缓存复用已在生产真实历史上验证。
