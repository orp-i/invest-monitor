...
线上核对通过：迁移前后持仓、结单成交、复盘档案数量及已实现净盈亏、累计费用均一致。休市实盘最近成交显示正确，跨月查询与手机布局正常。
...
盈亏历史改为北京时间每天12:00和00:00记录，每日一根基于实际样本的K线；旧5分钟记录保留后按日聚合。取消服务端5分钟采样、券商同步写盈亏历史和浏览器5分钟轮询。当前卡片仍支持券商SSE/手动/页面恢复读取，不追加图表样本。调度按保存的半日时段去重、失败重试、重启只补当前时段；采集时间保持真实，不伪造漏掉的过去记录。沿用原表和366天保留期，历史读取上限10000且使用索引/LIMIT。
