...
import { sopInferEffects } from "./review-sop.js";
...
export interface ExecutionLeg {
  ...
  /** Distinct broker closed-lot ids attached to this leg (0 when the broker gave none). */
  lots: number;
}
...
export function reviewExecutionDetails(entry: TradingCase) {
  const confirmed = userConfirmedGrouping(entry);
  // SOP-4 default applies only to groupings the user made or confirmed; broker flags on any fill switch it off.
  const sop = confirmed ? sopInferEffects(entry.fills) : null;
  const brokerEffect = (f: ReviewFill) => f.positionEffect ?? f.provenance?.action ?? null;
  const effectOf = (f: ReviewFill) => brokerEffect(f) ?? sop?.effects.get(f.id) ?? null;
  ...
  for (const [key, original] of groups) {
    ...
    const ambiguous = fills.some((f, i) => fills.some((g, j) => j !== i && (g.occurredAt === f.occurredAt || (f.timePrecision === "day" || g.timePrecision === "day") && brokerReportDay(g.occurredAt) === brokerReportDay(f.occurredAt)) && f.side !== g.side && !effectOf(f)));
    ...
    const openings: ReviewFill[] = [];
    for (const fill of fills) {
      const quantity = new Money(fill.quantity), signed = quantity.mul(fill.side === "buy" ? 1 : -1);
      const explicit = effectOf(fill), inferred = !brokerEffect(fill) && !!sop?.effects.has(fill.id);
      ...
      if (opened.gt(0) || closed.gt(0)) actions[fill.id] = (opened.gt(0) && closed.gt(0) ? `${executionAction(fill.side, "close")}；${executionAction(fill.side, "open")}` : executionAction(fill.side, opened.gt(0) ? "open" : "close")) + (inferred ? "（SOP 推断）" : "");
      // Openings confirmed by a flag, by the SOP default, or by date order inside a grouping the user confirmed.
      if (opened.gt(0) && (explicit || confirmed)) openings.push(fill);
    ...
    legs.push({ openingAt, openingDay, openingConfirmed: openings.length > 0, direction, directionLabel: directionLabel(fills[0]!.symbol, direction), multiplier: sizes.size === 1 ? fills[0]!.multiplier : null, key, symbol: fills[0]!.symbol, broker: reviewBroker(fills[0]!), currency,
      ...
      lots: new Set(fills.flatMap(f => f.lotId ? [f.lotId] : [])).size });
  }
  const currencies = [...new Set(entry.fills.map(f => f.currency))];
  // Broker open/close flags on every fill (closed lots, observed orders, statement actions) allow same-day
  // recognition without user confirmation; the label then carries the broker-evidence suffix.
  const brokerFlagged = entry.fills.length > 0 && entry.fills.every(f => !!brokerEffect(f));
  const strategy = identifyStrategy(legs, { sameDayConfirmed: confirmed, sameDayEvidence: brokerFlagged, sopInferred: sop !== null });
  return { legs, actions, strategy, direction: strategy.label, sop: sop ? { rule: sop.rule, note: sop.note } : null, fees: currencies.map(currency => ({ currency, total: exactTotal(entry.fills.filter(f => f.currency === currency).map(f => f.feeCost)) })) };
}
